...
namespace {
    ...
    VuforiaAppSession* instance = nil;
    ...
    NSString * SAMPLE_APPLICATION_ERROR_DOMAIN = @"vuforia_cordova_plugin";
}

@interface VuforiaAppSession ()
...
// VuforiaAppControl delegate (receives callbacks in response to particular
...
@implementation VuforiaAppSession
@synthesize viewport;

- (id)initWithDelegate:(id<VuforiaAppControl>) delegate
{
    self = [super init];
    if (self) {
...
- (void)initVuforiaInBackground
{
    ...
    @autoreleasepool {
        ...
        NSString *filePath = [rootPath stringByAppendingString:@"/www/license/vuforiaLicense.txt"];
        NSString *licenseKey = [NSString stringWithContentsOfFile:filePath encoding:NSUTF8StringEncoding error:nil];

        if(licenseKey == nil) {
            licenseKey = @"";
        }

        const char *key = [licenseKey cStringUsingEncoding:NSUTF8StringEncoding];
