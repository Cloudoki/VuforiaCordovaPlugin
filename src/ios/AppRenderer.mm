...
- (void) initRendering {
    ...
    self.vbShaderProgramID = [VuforiaAppShaderUtils createProgramWithVertexShaderFileName:@"Background.vertsh"
...
- (void)renderFrameVuforia
{
    ...
    for (int viewIdx = 0; viewIdx < viewList.getNumViews(); viewIdx++) {
        ...
        VuforiaAppUtils::multiplyMatrix(&rawProjectionMatrixGL.data[0], &eyeAdjustmentGL.data[0], &projectionMatrix.data[0]);
...
- (void) renderVideoBackground {
    ...
    if (Vuforia::Device::getInstance().isViewerActive())
    {
        ...
        VuforiaAppUtils::scalePoseMatrix(sceneScaleFactor, sceneScaleFactor, 1.0f, vbProjectionMatrix.data);
    }
    ...
    VuforiaAppUtils::checkGlError("Rendering of the video background failed");
}
